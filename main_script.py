from corpus import Document, NPChunkCorpus, WordOnlyNPChunkCorpus, POSOnlyNPChunkCorpus
from hmm import HMM
...
class HMMTest(TestCase):
    ...
    def split_np_chunk_corpus1(self, document_class):
        """Split the yelp review corpus into training, dev, and test sets"""
        sentences = WordOnlyNPChunkCorpus('np_chunking_wsj', document_class=document_class)
        seed(hash("np_chunk"))
        shuffle(sentences)
        return (sentences[:8936], sentences[8936:])

    def test_np_chunk_baseline1(self):
        """Test NP chunking with word and postag feature"""
        train, test = self.split_np_chunk_corpus1(Document)
        classifier = HMM()
        classifier.train(train)
        test_result = compute_cm(classifier, test)
        _, _, f1, accuracy = test_result.print_out()
    ...
    def split_np_chunk_corpus2(self, document_class):
        """Split the yelp review corpus into training, dev, and test sets"""
        sentences = POSOnlyNPChunkCorpus('np_chunking_wsj', document_class=document_class)
        seed(hash("np_chunk"))
        shuffle(sentences)
        return (sentences[:8936], sentences[8936:])

    def test_np_chunk_baseline2(self):
        """Test NP chunking with word and postag feature"""
        train, test = self.split_np_chunk_corpus2(Document)
        classifier = HMM()
        classifier.train(train)
        test_result = compute_cm(classifier, test)
        _, _, f1, accuracy = test_result.print_out()
